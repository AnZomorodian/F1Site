...
def telemetry():
    ...
    if not all([year, round_number, session, drivers]):
        ...
        return render_template('index_modern.html', years=f1_data.get_available_years())
    ...
    try:
        ...
        return render_template('telemetry_modern.html', 
...
def not_found_error(error):
    return render_template('index_modern.html', years=f1_data.get_available_years()), 404

@app.errorhandler(500)
def internal_error(error):
    logging.error(f"Internal server error: {str(error)}")
    flash('An internal error occurred. Please try again.', 'error')
    return render_template('index_modern.html', years=f1_data.get_available_years()), 500
